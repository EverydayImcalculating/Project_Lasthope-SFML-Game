	...
	: m_shared(l_shared)
{
	...
	RegisterState<State_ScoreBoard>(StateType::ScoreBoard);
	RegisterState<State_Name>(StateType::Name);
	RegisterState<State_Credits>(StateType::Credits);
	RegisterState<State_Howto>(StateType::Howto);
	RegisterState<State_Noob>(StateType::Noob);
}
...
void StateManager::Update(const sf::Time& l_time, sf::Event event) {
	...
	if (m_states.back().second->IsTranscendent() && m_states.size() > 1) {
		...
		for (; itr != m_states.end(); ++itr) {
			itr->second->Update(l_time, event);
		}
	}
	else {
		m_states.back().second->Update(l_time, event);
	}
}
...
void StateManager::SetName(std::string l_name)
{
	receivedName = l_name;
}
std::string StateManager::GetName() { return receivedName; }